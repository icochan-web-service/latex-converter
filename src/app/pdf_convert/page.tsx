...
export default function PdfConvertPage() {
  ...
  const convert = async () => {
    if (!pdfFile) return;
    if (!isSignedIn) {
      openSignIn({ fallbackRedirectUrl: "/pdf_convert" });
      return;
    }
    setLoading(true);
    try {
        ...
        >
          ...
          {isLoaded && (
            isSignedIn && usage ? (
              ...
              >
                <div style={{ fontSize: "12px", color: "#666666", marginBottom: "2px" }}>今月の変換回数</div>
                <div
                  style={{
                    fontSize: "17px",
                    fontWeight: "700",
              ...
              <SignInButton mode="modal" fallbackRedirectUrl="/pdf_convert">
                <button
                  style={{
                    background: "#0017C1",
                    color: "#fff",
